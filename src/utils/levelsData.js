...
export const loadLevels = async () => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error('❌ levelsData: Fetch failed!', { status: response.status, statusText: response.statusText, body: errorText });
            throw new Error(`Failed to load levels data: ${response.status} ${response.statusText}`);
        }

        const data = await response.json();
        console.log('📥 levelsData: Parse successful. Data type:', typeof data, 'IsArray:', Array.isArray(data));
        ...
        if (Array.isArray(data)) {
            ...
            throw new Error('Invalid levels data format: expected an array or an object with a "levels" array');
        }
