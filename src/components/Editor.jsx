  ...
  const renderKeyframesHeader = () => {
    ...
    return (
      <div className="keyframes-header">
        <span className="keyframes-name">
          @keyframes {level.keyframes.name}
        </span>
        {" {"}
      </div>
    );
  ...
  return (
    <div className="editor-container">
      <div className="description-box">
        ...
        {renderDescription()}
        {level.question && (
          <p className="question-highlight">{level.question}</p>
        )}
      </div>
      ...
      <div className="code-editor">
        <div className="line-numbers" id="lineNumbers">
          {level.code.map((_, index) => (
            <span key={index}>
              {index + EDITOR_CONSTANTS.LINE_NUMBER_START}
            </span>
          ))}
        </div>
        ...
        <pre className="code-content" id="codeContent" ref={codeContentRef}>
          ...
          {level.code.map((line, lineIndex) => {
            ...
            if (blank) {
              ...
              const inputIndex = level.blanks.findIndex(
                (b) => b.line === lineIndex,
              );
              ...
              return (
                <div key={lineIndex} className={EDITOR_CSS_CLASSES.CODE_LINE}>
                  {parts[0] && (
                    <span className={EDITOR_CSS_CLASSES.CODE_LINE_TEXT}>
                      {parts[0]}
                    </span>
                  )}
