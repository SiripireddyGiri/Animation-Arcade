...
import { validateName, validatePassword, validateMatch } from '../utils/validator';
...
const AuthModal = ({ isOpen, onClose }) => {
    ...
    return (
        <div id="loginOverlay" className="overlay" style={overlayStyle}>
            <div className="modal">
                <button id="closeModal" className="close-btn" onClick={onClose}>&times;</button>
                <div className="modal-brand">Animation Arcade</div>
                <h2 id="modal-title">{currentTab === TABS.LOGIN ? 'Login' : 'Sign Up'}</h2>

                <div className="tab-container" role="tablist" aria-label="Authentication">
                    <button
                ...
                {message.text && (
                    <div id="authMessage" className={`auth-message ${message.type}`}>{message.text}</div>
                )}

                <form id="authForm" className="auth-form" onSubmit={handleSubmit}>
                    {currentTab === TABS.SIGNUP && (
                        <div className={`form-group ${errors.name ? 'show-tooltip' : ''}`} data-tooltip={errors.name}>
                            <input
                                type="text"
                                id="name"
                                name="name"
                    ...
                    <div className={`form-group ${errors.email ? 'show-tooltip' : ''}`} data-tooltip={errors.email}>
                        <input
                            type="email"
                            id="email"
                            name="email"
                    ...
                    <div className={`form-group ${errors.password ? 'show-tooltip' : ''}`} data-tooltip={errors.password}>
                        <input
                            type="password"
                            id="password"
                            name="password"
                    ...
                    {currentTab === TABS.SIGNUP && (
                        <div className={`form-group ${errors.confirmPassword ? 'show-tooltip' : ''}`} data-tooltip={errors.confirmPassword}>
                            <input
                                type="password"
                                id="confirmPassword"
                ...
                <div className="auth-footer">
                    <span id="authFooterText">
                        <span id="footerActionText">
                            {currentTab === TABS.LOGIN ? 'New user?' : 'Already have an account?'}
                        </span>{' '}
                        <a
                            href="#"
                            id="footerActionLink"
                            onClick={(e) => {
                                e.preventDefault();
                                handleTabSwitch(currentTab === TABS.LOGIN ? TABS.SIGNUP : TABS.LOGIN);
                            }}
                        >
