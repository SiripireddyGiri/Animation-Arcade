...
import { ROUTES, EDITOR_CONSTANTS, EDITOR_CSS_CLASSES, SELECTORS } from '../utils/constants';
...
const GamePage = () => {
    ...
    const handleValidate = () => {
        ...
        if (allCorrect) {
            ...
            updatedInputs.forEach((input, idx) => {
                ...
                if (line.includes(':')) {
                    const candidate = line.replace(EDITOR_CONSTANTS.BLANK_PLACEHOLDER, input.value).trim();
    ...
    if (!level) {
        console.warn('⚠️ GamePage: level is undefined.', { currentLevel, levelsCount: levels.length });
        return (
            <div style={{ padding: '2rem', textAlign: 'center' }}>
                <h2>Level not found</h2>
                <p>Current Level Index: {currentLevel}</p>
                <p>Total Levels: {levels.length}</p>
                {levels.length === 0 && <p>Levels data is empty. Please check the network tab or console for load errors.</p>}
            </div>
        );
    }
